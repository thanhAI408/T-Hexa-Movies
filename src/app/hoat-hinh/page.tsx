...
export const metadata: Metadata = {
  title: "Hoạt hình",
  description: "Danh sách phim hoạt hình, anime, cartoon từ nhiều nguồn.",
};
...
export default async function AnimationPage({ searchParams }: Props) {
  const { page } = await searchParams;
  const currentPage = Math.max(1, parseInt(page ?? "1", 10));
  const offset = (currentPage - 1) * 24;
  const movies = await listMovies({ type: "animation", limit: 24, offset });

  return (
    <div className="container py-8">
      <div className="mb-6 flex items-center gap-3">
        <PlaySquare size={28} className="text-[#c97df5]" />
        <h1 className="text-2xl font-bold text-white">Hoạt hình</h1>
      </div>
      ...
      {movies.items.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <PlaySquare size={64} className="mb-6 text-[#4a5568]" />
          <h2 className="mb-3 text-xl font-semibold text-white">Chưa có hoạt hình</h2>
